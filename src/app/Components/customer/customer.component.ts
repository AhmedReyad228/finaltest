...
export class CustomerComponent implements OnInit{
  ...
  selectedCustomerTransactions: any[] = [];
    ...
    ngOnInit(): void {
      this._DataService.getData().subscribe(custData => {
        this._DataService.getData().subscribe(transData => {
          this.combinedData =
          transData.transactions.map((transaction:any) => {const customer = custData.customers.find((cust:any) => cust.id == transaction.customer_id);
